...
let OAuthVerifyService = OAuthVerifyService_1 = class OAuthVerifyService {
    ...
    async verifyGoogle(token) {
        try {
            this.logger.log(`Google token preview: ${token.substring(0, 30)}... parts: ${token.split('.').length}`);
            ...
            return {
                ...
                image: payload.picture || (await this.fetchGooglePicture(token)),
            };
        }
        catch (e) {
    ...
    async fetchGooglePicture(idToken) {
        try {
            const data = await httpGet(`https://oauth2.googleapis.com/tokeninfo?id_token=${idToken}`);
...
//# sourceMappingURL=oauth-verify.service.js.map